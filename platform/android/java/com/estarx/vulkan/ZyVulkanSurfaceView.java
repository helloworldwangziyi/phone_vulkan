 ...
 * 自身不含任何业务逻辑（"薄壳"的含义）。
 */
public class ZyVulkanSurfaceView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private final Choreographer.FrameCallback frameCallback = new Choreographer.FrameCallback() {
        ...
        public void doFrame(long frameTimeNanos) {
            ...
            ZyNativeBridge.nativeBeginFrame(frameTimeNanos);
            Choreographer.getInstance().postFrameCallback(this);
        }
    };

    // 构造函数①：代码里 new ZyVulkanSurfaceView(context) 时走这个。
    public ZyVulkanSurfaceView(Context context) {
        super(context);
        init();
    }

    // 构造函数②：XML 布局反射创建时走这个（本项目 zy_activity_main.xml 走的就是它）。
    // attrs 装着 XML 标签上写的属性（android:id、layout_width 等）。
    // 两个构造都必须提供，否则反射或手动 new 总有一边会崩。
    public ZyVulkanSurfaceView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        ...
        ZyNativeBridge.nativeInit(holder.getSurface());
        surfaceReady = true;
    ...
    public void surfaceChanged(SurfaceHolder holder, int format, int width, int height) {
        ZyNativeBridge.nativeResize(width, height); // 告诉 native 画板新尺寸
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_DOWN) {
            ...
            ZyNativeBridge.nativeOnTouch(action, activePointerId, event.getX(0),
                    event.getY(0), eventTimeNanos);
        } else if (action == MotionEvent.ACTION_MOVE && activePointerId != NO_POINTER) {
            final int pointerIndex = event.findPointerIndex(activePointerId);
            if (pointerIndex >= 0) {
                ZyNativeBridge.nativeOnTouch(action, activePointerId,
            ...
            ZyNativeBridge.nativeOnTouch(MotionEvent.ACTION_UP, activePointerId,
                    event.getX(pointerIndex), event.getY(pointerIndex), eventTimeNanos);
            activePointerId = NO_POINTER;
        } else if (action == MotionEvent.ACTION_CANCEL && activePointerId != NO_POINTER) {
            ZyNativeBridge.nativeOnTouch(action, activePointerId, 0.0f, 0.0f, eventTimeNanos);
            activePointerId = NO_POINTER;
        }
