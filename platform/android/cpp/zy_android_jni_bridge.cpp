// ============================================================================
// JNI 实现层：Java 薄壳(ZyNativeBridge)与 core 之间的翻译层。
//
// 本文件即 JNI 书写规则的范例，规则逐条如下：
//
// 1. 命名约定（静态注册）：函数名 = Java_<包名>_<类名>_<方法名>，
//    包名中的 . 换成 _，方法名中的 _ 要转义成 _1。
//    例：com.estarx.vulkan.ZyNativeBridge.nativeInit
//        → Java_com_estarx_vulkan_ZyNativeBridge_nativeInit。
...
// 事件经 evk::dispatchEvent 回流给 App 注册的 EventFunc（见 zy_app_lifecycle.h）。
// ============================================================================
...
#include "evk/zy_vulkan_renderer.h"
...
// zy_android_vulkan_platform.cpp 以 C 链接导出的工厂函数（规则 2 的同款应用）：
...
// Java: ZyNativeBridge.nativeInit(Surface)
// surface 创建就绪时调用（可能多次：退后台重建）。幂等：已有渲染器直接返回。
extern "C" JNIEXPORT void JNICALL
Java_com_estarx_vulkan_ZyNativeBridge_nativeInit(JNIEnv* env, jclass /*clazz*/, jobject surface) {
    // logcat sink 由平台壳注入；core 默认只提供 stdout（见 evk/zy_log.h）。
...
// Java: ZyNativeBridge.nativeResize(int, int)
extern "C" JNIEXPORT void JNICALL
Java_com_estarx_vulkan_ZyNativeBridge_nativeResize(JNIEnv* /*env*/, jclass /*clazz*/, jint width, jint height) {
...
extern "C" JNIEXPORT void JNICALL
Java_com_estarx_vulkan_ZyNativeBridge_nativeBeginFrame(JNIEnv* /*env*/, jclass /*clazz*/,
                                                     jlong frameTimeNanos) {
    evk::beginFrame(static_cast<int64_t>(frameTimeNanos));
}

// Java: ZyNativeBridge.nativeOnTouch(int, int, float, float, long)
// 本层只把 Android MotionEvent 动作翻译成跨平台 PointerAction；
// eventTimeNanos 来自 MotionEvent.getEventTime()，供 core 计算滑动速度。
extern "C" JNIEXPORT void JNICALL
Java_com_estarx_vulkan_ZyNativeBridge_nativeOnTouch(JNIEnv* /*env*/, jclass /*clazz*/,
...
// Java: ZyNativeBridge.nativeDestroy()
extern "C" JNIEXPORT void JNICALL
Java_com_estarx_vulkan_ZyNativeBridge_nativeDestroy(JNIEnv* /*env*/, jclass /*clazz*/) {
