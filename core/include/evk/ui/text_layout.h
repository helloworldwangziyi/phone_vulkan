 ...
 * 换行规则从简（仅简体/繁体中文 + 英文）：
 *   - '\n' 强制断行；
 *   - CJK 码点（0x2E80–0x9FFF、0x3000–0x303F、0xFF00–0xFFEF、
 *     0x20000–0x2A6DF；简繁同在这些区块）前后任意断；
 *   - ASCII 字母/数字按词断：行内最后一个空格是可断点，超宽回退到它；
 *     断行处的空格直接丢弃（不留在行尾、也不带进下一行），行尾多余
 *     空白在成行时统一修剪；单词自身超宽时硬断；
 *   - 行首空白跳过。
 * 不做 shaping/ICU/避头尾等复杂规则。
 *
