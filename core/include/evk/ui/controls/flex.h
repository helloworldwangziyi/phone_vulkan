...
/**
 * @file flex.h
 * @brief Flex 线性布局容器（Flutter Column/Row 的对应物）。
 *
 ...
 *
 * @note 规则（v1）：主轴方向每个子视图必须 main≥0（固定）或 weight>0（弹性）
 *       二选一；不测量子视图固有尺寸。
 */
...
/**
 * @brief 交叉轴对齐方式（cross≥0 时生效）。
 */
typedef enum esx_flex_align {
    ESX_FLEX_ALIGN_STRETCH = 0, ///< 交叉轴占满（cross<0 时的默认行为）
    ESX_FLEX_ALIGN_START   = 1, ///< 交叉轴贴前
    ESX_FLEX_ALIGN_CENTER  = 2, ///< 交叉轴居中
    ESX_FLEX_ALIGN_END     = 3, ///< 交叉轴贴后
} esx_flex_align;

/**
 * @brief Flex 子项排布参数（主轴/交叉轴尺寸与 margin）。
 */
typedef struct esx_flex_child {
    ...
    float margin_main_before; ///< 主轴前外边距
    float margin_main_after; ///< 主轴后外边距
 ...
 * @brief 设置/更新子视图的排布参数。
 *
 * @param child 须 parent=0 创建、尚未挂载（挂载进 flex 后排布由容器接管）；
 *        已是 flex 子视图时调用则更新 spec 并重排
 * @param spec 排布参数，传 NULL 用全零默认
 */
