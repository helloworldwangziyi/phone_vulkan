...
test_binary="${TMPDIR:-/tmp}/semantics_test"
obj_dir="${TMPDIR:-/tmp}/semantics_test_obj"
cxx=${CXX:-c++}
cc=${CC:-cc}
sdk_flags=
...
"$cxx" $sdk_flags -std=c++17 -Wall -Wextra -Werror \
    ...
    $unibreak_objs \
    "$repo_root/tests/semantics_test.cpp" \
    ...
    "$repo_root/core/src/ui/text_layout.cpp" \
    ...
    "$repo_root/core/src/ui/inherited_widget.cpp" \
    "$repo_root/core/src/ui/semantics.cpp" \
    ...
    "$repo_root/core/src/ui/controls/semantics_widget.cpp" \
...
# 文本测量需要两个字体文件路径作为运行参数（argv[1]=拉丁、argv[2]=中文）。
"$test_binary" \
